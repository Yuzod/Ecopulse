 ...
 *  D12 -> KY038 (Sensor de Sonido - salida analógica)
 *  D14 -> MQ135 (Calidad del Aire)
...
void leerSensores() {
  ...
  // KY038: promedio de 10 muestras para mayor estabilidad
  long suma = 0;
  for (int i = 0; i < 10; i++) {
    suma += analogRead(PIN_KY038);
    delay(5);
  }
  valorKY038 = suma / 10;
}
...
String clasificarSonido(int v) {
  if (v < 100)       return "SILENCIO";
  else if (v < 300 ) return "BAJO";
  else if (v < 700) return "NORMAL";
  else if (v < 1500) return "ALTO";
